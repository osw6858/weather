...
**문제:** 
- "ㄱㄴ" 입력으로 "강남구"를 검색할 수 있어야 UX가 좋음
- 약 2만 건의 행정구역 데이터를 매 입력마다 전체 순회하면 성능 저하 우려

**해결:**
- es-hangul 라이브러리로 초성 검색 구현
- maxResults를 설정하여 검색 결과가 10개 도달하면 즉시 순회 중단하여 불필요한 연산 방지
- 초성이 포함되지 않은 일반 검색어는 초성 변환 로직을 스킵

**향후 개선 가능성:** 
- 검색 결과를 훨씬 더 많이 반환해야 할 경우 Web Worker로 분리하여 메인 스레드 블로킹 방지 고려
