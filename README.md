...
**문제:** 
- 사용자 편의를 위해 **한글 초성 검색(예: "ㄱㄴ" → "강남구")** 기능을 `es-hangul` 라이브러리로 구현
- 약 2만 건의 행정구역 데이터를 대상으로 매 키보드 입력(onChange)마다 초성 변환 및 `replaceAll`, `split` 등의 무거운 문자열 연산이 발생
- 이로 인해 빠른 타이핑 시 저사양 기기에서의 입력 반응이 느려지는 현상 우려
